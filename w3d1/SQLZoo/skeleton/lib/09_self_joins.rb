...
def craiglockhart_to_sighthill
  ...
  execute(<<-SQL)
    SELECT DISTINCT
      service1.num,
      service1.co,
      service1.transfer_stop_name,
      service2.num,
      service2.co
    FROM (
      SELECT
        stop_b.name AS transfer_stop_name,
        b.company AS co,
        b.num AS num
      FROM
        routes a
    ...
    JOIN (
      SELECT
        stop_a.name AS transfer_stop_name,
        b.company AS co,
        b.num AS num
      FROM
        routes a
      ...
      ON
        stop_b.id = b.stop_id
      WHERE
        stop_b.name = 'Sighthill'
    ) service2
    ON
      service1.transfer_stop_name = service2.transfer_stop_name
    WHERE
      service1.num <> service2.num
    OR
      service1.co <> service2.co
